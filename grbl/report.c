...
/*
该文件用作Grbl的主要反馈接口。 任何传出的数据，例如
作为协议状态消息，反馈消息和状态报告，存储在此处。
在大多数情况下，这些函数主要是从protocol.c方法调用的。 如果一个
需要不同的样式反馈（即JSON），然后用户可以更改以下内容
满足他们需求的方法。
*/
...
//处理流接口和人工反馈的主要确认协议响应。
//对于每个传入的行，此方法响应成功命令的'ok'或
//'错误：'表示行中的某些错误事件或某些关键系统错误
//操作 错误事件可以源自g代码解析器，设置模块或异步
...
//打印Grbl从用户收到的字符串行，该字符串已经过预解析，
//已被发送到由grbl执行的protocol_execute_line（）例程。
...
//打印实时数据。 此功能可捕获步进子程序的实时快照
//以及CNC机床的实际位置。 用户可以将以下功能更改为他们的
//特定需求，但所需的实时数据报告必须尽可能短。 这是
//需要尽量减少计算开销并允许grbl保持平稳运行，
//特别是在具有快速，短线段和高频报告（5-20Hz）的g代码程序中。
